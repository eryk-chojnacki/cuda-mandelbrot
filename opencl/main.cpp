...
extern "C" const char *opencl_program;
...
int runCL(int width, int height)
{
	...
	check_succeeded("Creating buffer", err);
  	...
  	kernel = load_kernel_from_string(context, opencl_program);
	...
	double generation_time_us = generation_time.microseconds();

	std::cout << "gen time us: " << generation_time_us << " write output us: " << write_output.microseconds() << std::endl;
