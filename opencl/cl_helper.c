...
cl_kernel load_kernel_from_string(cl_context context, const char *program_source) {
...
//	check_succeeded("Getting device IDs", err);
  ...
  // nvidia drivers require a platform
  ...
  check_succeeded("Getting platform IDs", err);
  //printf("found %u platforms\n", num_platforms);
  err = clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_ALL, 16, devices, num_devices);
  check_succeeded("Getting device IDs", err);
